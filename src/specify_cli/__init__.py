...
TAGLINE = "GitHub Spec Kit - Spec-Driven Development Toolkit | Russian Edition"
...
def load_template_from_local(
    ai_assistant: str, local_dir: Path, *, script_type: str = "sh", verbose: bool = True
) -> tuple[Path, dict]:
    ...
    if not matches:
        console.print(
            f"[red]No local template found[/red] matching pattern: [bold]{pattern}[/bold]"
        )
